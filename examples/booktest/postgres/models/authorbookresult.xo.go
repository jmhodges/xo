...
func AuthorBookResultsByTags(db XODB, tags StringSlice) ([]*AuthorBookResult, error) {
	return AuthorBookResultsByTagsContext(context.Background(), db, tags)
}

// AuthorBookResultsByTagsContext runs a custom query, returning results as AuthorBookResult.
func AuthorBookResultsByTagsContext(ctx context.Context, db XODB, tags StringSlice) ([]*AuthorBookResult, error) {
	var err error
	...
	// run query
	XOLog(sqlstr, tags)
	q, err := db.QueryContext(ctx, sqlstr, tags)
	if err != nil {
		return nil, err
	}
	defer q.Close()
